...
#           for j in xrange(n):
#            	for k in xrange(m):
#             		reconstruct[k] = reconstruct[k] + (theta.getT() * X[j,:].getT() - y[j]) * X[j, k]
#           for l in xrange(m):
#            	theta[l] = theta[l] - reconstruct[l] * (self.alpha / n)
            theta = (theta.getT() - ((X * theta) - y).getT() * (self.alpha / n) * X).getT()
        return theta
    ...
    def computeCost(self, X, y, theta):
    	...
    	n,d = X.shape
    	cost_eqn = ((X * theta) - y).getT() * ((X * theta) - y) * 1 / (2 * n)
    	return cost_eqn[0,0]
